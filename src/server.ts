...
const server=createServer({requestTimeout:15000,headersTimeout:10000},async(req,res)=>{
 ...
 try{
  ...
  const files:Record<string,string>={'/':'index.html','/app.js':'app.js','/style.css':'style.css','/favicon.svg':'favicon.svg','/robots.txt':'robots.txt'};const file=files[path];if(!file)return json(res,404,{error:'Not found'});
