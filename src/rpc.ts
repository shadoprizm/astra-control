...
export class Rpc extends EventEmitter {
  ...
  pending=new Map<number,{resolve:(x:any)=>void,reject:(e:Error)=>void,timer:NodeJS.Timeout}>();
  next=0; connecting?:Promise<void>; ready=false; generation='';
  constructor(public host:HostConfig){super();}
  async connect(){
  ...
  private async start(){
    ...
    const p=this.host.ssh ? spawn('ssh',['-T','-o','BatchMode=yes','-o','ConnectTimeout=8','-o','ServerAliveInterval=15','-o','ServerAliveCountMax=2',this.host.ssh,`exec ${quote(this.host.codex)} app-server`]) : spawn(this.host.codex,['app-server']);
    this.proc=p; this.generation=randomUUID();
    const rl=createInterface({input:p.stdout});
    rl.on('line',line=>{try{const m=JSON.parse(line); if(m.id!=null && !m.method){const v=this.pending.get(m.id); if(v){clearTimeout(v.timer);this.pending.delete(m.id);m.error?v.reject(new Error(m.error.message)):v.resolve(m.result);}}else{this.emit('event',m,this.generation);}}catch{}});
