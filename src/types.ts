...
export interface Task { key:string; hostId:string; id:string; title:string; cwd:string; projectId?:string; branch?:string; updatedAt:number; observedAt:number; turnId?:string; turnStatus?:string; status:string; owned:boolean|null; latest?:Message; messages:Message[]; model?:string; error?:string; managed?:boolean; watched?:boolean; }
export interface Project { id:string|null; hostId:string; name:string; roots:string[]; source:'codex'|'history'; recencyAt?:number|null; }
...
export interface WorkItem {
 key:string; id:string; kind:WorkKind; title:string; status:WorkStatus; statusConfidence:StatusConfidence;
 updatedAt:number; observedAt:number; createdAt?:number; archived:boolean; pinned:boolean; watched:boolean;
 latestExcerpt?:string; sourceRefs:WorkSourceRef[]; execution:ExecutionMetadata;
 // Compatibility fields keep the existing Codex-oriented UI and controls narrow.
 hostId:string; cwd:string; projectId?:string; branch?:string; latest?:Message; messages:Message[];
 model?:string; error?:string; managed?:boolean; owned:boolean|null; turnId?:string; turnStatus?:string;
}
