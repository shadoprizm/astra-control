...
const mutationOrigins=new Set(config.auth?.mode==='cloudflare-access'?[publicOrigin!]:[`http://localhost:${config.port}`,`http://127.0.0.1:${config.port}`,...(publicOrigin?[publicOrigin]:[])]);
...
function optionalText(value:any,name:string,max=12000):string|null{return value==null||value===''?null:text(value,name,max);}
...
engine.on('invalidation',(data:any)=>{for(const res of peers)res.write(`event: invalidate\ndata: ${JSON.stringify(data)}\n\n`);});
const server=createServer({requestTimeout:15000,headersTimeout:10000},async(req,res)=>{
 ...
 try{
  ...
  if(req.method==='POST'){
   ...
   if(workWatchMatch){if(typeof b.value!=='boolean')throw new Error('Invalid watch value');engine.watch(decodeURIComponent(workWatchMatch[1]),b.value);return json(res,200,{ok:true});}
   ...
   if(path==='/api/actions/resolve-many'){if(!Array.isArray(b.ids)||!b.ids.length||b.ids.length>250)throw new Error('Choose between 1 and 250 inbox items');const ids:string[]=[...new Set((b.ids as unknown[]).map(id=>text(id,'action',100)))],actions=store.actions(),chosen=ids.map(id=>actions.find(a=>a.id===id));if(chosen.some(a=>!a))throw new Error('One or more inbox items no longer exist');if(chosen.some(a=>a.kind==='approval'&&a.status!=='expired'))throw new Error('Answer live approval requests individually');store.resolveMany(ids);engine.emit('change');return json(res,200,{ok:true,resolved:ids.length});}
   ...
   if(path==='/api/hosts/refresh')return json(res,200,await engine.refreshMachine(text(b.hostId,'machine',100)));
  }
  if(path.startsWith('/api/'))return json(res,404,{error:'Not found'});
  if(req.method!=='GET'&&req.method!=='HEAD')return json(res,405,{error:'Method not allowed'});
  const files:Record<string,string>={'/':'index.html','/app.js':'app.js','/workspace.js':'workspace.js','/conversation.js':'conversation.js','/style.css':'style.css','/conversation.css':'conversation.css','/favicon.svg':'favicon.svg','/robots.txt':'robots.txt'};const file=files[path];if(!file)return json(res,404,{error:'Not found'});
