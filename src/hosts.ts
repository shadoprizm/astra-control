...
export class Host {
 ...
 inventoryCount=0;
 projects:Project[]=[]; projectsError=''; projectsRefreshedAt=0; projectsPolling=false;
 ...
 async worktree(cwd:string,title:string):Promise<{cwd:string;branch:string;sourceRoot:string}>{return this.python({method:'worktree',cwd,title});}
 async refreshProjects(force=false){
  if(this.projectsPolling||!force&&Date.now()-this.projectsRefreshedAt<60000)return;
  this.projectsPolling=true;
  try{
   const projects:Project[]=[];let cursor:string|null=null,pages=0;
   do{
    const r=await this.rpc.call('project/list',{cursor,limit:100,sortKey:'position',sortDirection:'asc'});
    if(!Array.isArray(r?.data))throw new Error('Codex returned an invalid project list');
    for(const p of r.data){
     const roots:string[]=Array.isArray(p.roots)?p.roots.flatMap((x:any)=>typeof x?.path==='string'&&x.path.trim()?[x.path]:[]):[];
     if(typeof p.id==='string'&&typeof p.name==='string'&&roots.length)projects.push({id:p.id,hostId:this.config.id,name:p.name,roots:[...new Set(roots)],source:'codex',recencyAt:typeof p.recencyAt==='number'?p.recencyAt:null});
    }
    cursor=typeof r.nextCursor==='string'&&r.nextCursor?r.nextCursor:null;pages++;
   }while(cursor&&pages<20);
   this.projects=projects;this.projectsError='';
  }catch(e){this.projectsError=(e as Error).message;}
  finally{this.projectsRefreshedAt=Date.now();this.projectsPolling=false;}
 }
