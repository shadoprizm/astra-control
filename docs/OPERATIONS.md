...
Finish or pause dashboard-managed work before restarting or moving the hub: the current version owns App Server subprocesses, so stopping it closes its runtime connections. Pending permission IDs become invalid after restart and must never be replayed. For a dashboard-managed task, the owner can use the expired decision panel to ask the agent for a fresh request. Never retry an uncertain action automatically.
