...
test('demo selection and mutation guard are explicit',()=>{
 ...
 for(const path of ['/api/approval','/api/approval/reissue','/api/send','/api/pause','/api/archive','/api/create','/api/chat','/api/hosts/refresh','/api/shadow-analysis/run'])assert.equal(demoRejectsMutation(path),true,path);
